...
import { GameScraper, GameType, ScrapedPrice } from '../interfaces/scraper.interface';
  ...
  async search(query: string, cc = 'us'): Promise<ScrapedPrice[]> {
    try {
      const { data } = await firstValueFrom(
        this.http.get('https://store.steampowered.com/api/storesearch/', {
      ...
      for (const item of data.items.slice(0, 60)) {
        try {
          ...
          const appData = details?.[item.id]?.data;
          ...
          else if (steamType === 'bundle' || steamType === 'sub') gameType = 'bundle';
          ...
          results.push({
            ...
            originalPrice: priceData.initial !== priceData.final
      ...
      this.logger.error(`Steam search failed: ${error.message}`);
      return [];
    }
  }
}
